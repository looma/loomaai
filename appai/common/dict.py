...
class Dictionary:
    ...
    def get_ch_data(self, ch, field):
        ...
        search = re.search(
            r"([1-9]|10|11|12)(EN|S|SF|M|SS|N|H|V|CS)([0-9]{2}\.[0-9]{2})?",
            ch,
            re.IGNORECASE,
        )
        if field == "grade":
        ...
        try:
            ...
            for word in filtered_text:
                ...
                if entry is None and len(word) > 2 and self.has_numbers(word) == False:
                    ...
                    if (
                        "Definition Not Found" or "Please provide the word"
                    ) not in definition:
                    ...
                    entry is not None
                    and len(word) > 2
                    and self.has_numbers(word) == False
                ):
                    # query = {"en": word}
                    # entry = collection.find_one(query)
                    ...
                    if status == "changed":
                        ...
                        if (check not in entry["ch_id"] and grade_new < grade_ori) or (
                            check not in entry["ch_id"]
                            and grade_new == grade_ori
                            and number_new < number_ori
                        ):
