...
def translate_lessons(db: Database, llm, missing_only: bool):
    filter_q = {}
    if missing_only:
        filter_q = {"translated": None}

    with alive_progress.alive_bar(
        db.get_collection("lessons").count_documents(filter_q)
    ) as progress_bar:
...
def translate_one_lesson(db: Database, lesson, llm):
